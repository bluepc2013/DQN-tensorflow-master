...
class Agent(BaseModel):
  ...
  def train(self):
    ...
    # @num_game是游戏次数；@ep_reward是一次游戏中的总得分;这些数据全部是用作统计
    ...
    for self.step in tqdm(range(start_step, self.max_step), ncols=70, initial=start_step):
      ...
      if self.step == self.learn_start:
        num_game, self.update_count, ep_reward = 0, 0, 0.
        total_reward, self.total_loss, self.total_q = 0., 0., 0.
        ep_rewards, actions = [], []  # @ep_rewards 用来记录每次游戏的得分，用作统计数据
      ...
      # if game over, then reset it.
      if terminal:
        screen, reward, action, terminal = self.env.new_random_game()

        num_game += 1  # 记录游戏次数
        ep_rewards.append(ep_reward) # 将本次游戏的最终得分记录下来，下面用来做统计
        ep_reward = 0.
      else:
        ep_reward += reward # 本次游戏获得的分数
      ...
      if self.step >= self.learn_start:
        if self.step % self.test_step == self.test_step - 1:
          ...
          try:
            max_ep_reward = np.max(ep_rewards)  # 开始游戏以来获得的最高分
  ...
  def q_learning_mini_batch(self):
    ...
    """
    # ReplayMemory中存储的episode至少要大于4，否则没法训练
  ...
  def build_dqn(self):
    ...
    ?In this model, predition net is working as max_a' Q(s',a'). target net is working as Q(s,a).
    """
    ...
    self.load_model() #load checkpoint data
    self.update_target_q_network()
